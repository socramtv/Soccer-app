...
def extraer_eventos(url_eventos):
    ...
    try:
        ...
        # PASO 1: Identificar de forma unívoca todas las filas de partidos reales
        elementos_hora = []
        ...
        for el_hora in elementos_hora:
            ...
            if fila and len(fila.text) < 1500:
                tarjetas_validas.add(fila)

        print(f"🔍 Encontradas {len(tarjetas_validas)} filas de partidos para clasificar...")

        # PASO 2: Recorrido lineal y secuencial de arriba a abajo (State Machine)
        current_fecha = "Hoy"
        current_liga = "Otros Deportes"
        dias_semana = ['lunes', 'martes', 'miércoles', 'miercoles', 'jueves', 'viernes', 'sábado', 'sabado', 'domingo', 'hoy', 'mañana', 'ayer']
        
        # Analizamos todas las etiquetas en su estricto orden de aparición en el HTML
        for el in soup.find_all(['h2', 'h3', 'h4', 'div', 'tr', 'p']):
            txt = el.text.strip()
            clases = "".join(el.get('class', [])).lower() if el.get('class') else ""
            
            # A. ¿Es una cabecera de FECHA?
            ...
            if (contiene_dia or es_clase_fecha or contiene_mes) and len(txt) < 100 and " - " not in txt and not re.search(r'\b\d{2}:\d{2}\b', txt):
                ...
                current_liga = "Otros Deportes" # Reseteamos competición al cambiar de día por seguridad
                continue
                
            # B. ¿Es una cabecera de COMPETICIÓN / DEPORTE?
            tiene_clase_liga = any(x in clases for x in ['torneo', 'liga', 'competicion', 'copete', 'deporte', 'deportes', 'titulo', 'head', 'cabecera', 'categoria'])
            es_h_deporte = el.name in ['h2', 'h3', 'h4']
            
            if (tiene_clase_liga or es_h_deporte) and len(txt) < 100 and " - " not in txt and not re.search(r'\b\d{2}:\d{2}\b', txt) and txt != current_fecha:
                img_torneo = el.find('img')
                if img_torneo and img_torneo.get('alt'):
                    current_liga = img_torneo.get('alt', '').strip()
                else:
                    current_liga = txt
                current_liga = re.sub(r'\s+', ' ', current_liga).strip()
                continue
                
            # C. ¿Es una de nuestras filas de partidos identificadas?
            if el in tarjetas_validas:
                tarjetas_validas.remove(el) # Evitamos duplicados por duplicidad de nodos anidados
                ...
                try:
                    ...
                    # Separar de forma quirúrgica imágenes (Escudos vs Logotipos de TV)
                    ...
                    # Guardamos el partido con sus variables secuenciales exactas
                    if equipo_local and equipo_local != "Equipo Local":
                        eventos.append({
                            'hora': hora,
                            'fecha': current_fecha,
                            'liga': current_liga if current_liga else "Otros Deportes",
        ...
        # Eliminación final de duplicados por seguridad de renderizado
        eventos_unicos = []
        vistos_ev = set()
        for ev in eventos:
        ...
        print(f"✅ Éxito absoluto: {len(eventos_unicos)} partidos clasificados perfectamente de forma lineal.")
